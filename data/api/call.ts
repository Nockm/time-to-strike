...
import type { ValidHttpResponse } from './httpResponse.ts';
import { getDirname } from '../util/pathUtil.ts';
import { isValidHttpResponse } from './httpResponse.ts'; // eslint-disable-line no-duplicate-imports
...
export async function call (url: string, opts: CallOpts = {
    ...
    'useLocal': true,
    'useRemote': true,
}): Promise<ValidHttpResponse | null> {
    if (opts.useLocal) {
        const obj = readCache(url);

        if (isValidHttpResponse(obj)) {
